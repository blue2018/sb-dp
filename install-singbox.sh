...
EOF
	...
	# 【优化4】统一异步启动与验证逻辑
	(
	    if [ "$OS" = "alpine" ]; then RC_NO_DEPENDS=yes rc-service sing-box stop >/dev/null 2>&1 || true; sleep 1; RC_NO_DEPENDS=yes rc-service sing-box start >/dev/null 2>&1 || true
        else systemctl stop sing-box >/dev/null 2>&1 || true; sleep 1; systemctl start sing-box >/dev/null 2>&1 || true; fi
