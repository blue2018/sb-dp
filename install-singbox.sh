...
apply_warp_config() {
    ...
    # 1. 预处理：清理旧的 WARP 相关配置并开启 Inbound 嗅探
    # 无论启用还是禁用，我们都确保 inbounds 开启了 sniffing，这是域名分流的前提
    jq '
    .inbounds[0] += {"sniff": true, "sniff_override_destination": true} |
    .outbounds //= [] | 
    .route //= {} | 
    .route.rules //= [] |
    .outbounds |= map(select(.tag != "warp-out")) |
    .route.rules |= map(select(.outbound != "warp-out" and .tag != "dns-rule"))
    ...
    if [ "$action" = "enable" ]; then
        ...
        # 2. 注入新的 WARP 出站和路由规则
        # 添加了 dns-out 出站处理，确保 DNS 请求能命中路由
        jq --arg priv "$priv" '
        .outbounds = [{
            "type": "wireguard",
            "tag": "warp-out",
