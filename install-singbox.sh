...
get_network_info() {
    ...
    [ -n "$RAW_IP4" ] && succ "IPv4: $RAW_IP4 [检测成功]" || warn "IPv4: 不可用"
    [ "$IS_V6_OK" = "true" ] && succ "IPv6: $RAW_IP6 [检测成功]" || warn "IPv6: 不可用"
}
...
create_config() {
    ...
    local mem=$(probe_memory_total); : ${mem:=512}; local timeout="30s"
