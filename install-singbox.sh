...
get_network_info() {
    ...
    # 获取 IPv4：本地排除内网 -> API 兜底 (用 || true 防止 set -e 中断)
    v4_raw=$(ip -4 addr show scope global | awk '$1=="inet" && $2 !~ /^(127|10|172\.(1[6-9]|2[0-9]|3[0-1])|192\.168)\./ {print $2}' | cut -d/ -f1 | head -n1)
    [ -z "$v4_raw" ] && v4_raw=$(curl -4sL --max-time 3 api.ipify.org || curl -4sL --max-time 3 ifconfig.me || echo "")

    # 获取 IPv6：本地排除临时地址 -> API 兜底
    v6_raw=$(ip -6 addr show scope global | grep -vE "temporary|mngtmpaddr" | awk '/inet6 /{print $2}' | cut -d/ -f1 | head -n1)
    [ -z "$v6_raw" ] && v6_raw=$(curl -6sL --max-time 3 api6.ipify.org || curl -6sL --max-time 3 ifconfig.co || echo "")
    ...
    [ -n "$RAW_IP6" ] && ping6 -c1 -W1 2606:4700:4700::1111 >/dev/null 2>&1 && v6_ok="\033[32m✓\033[0m" || true
