...
create_config() {
    ...
    # 构造 Argo Inbound (xhttp stream-up 模式，替代 httpupgrade，兼容 CF 隧道转发)
    local ARGO_IN=""
    if [ -n "$A_TOKEN" ] && [ -n "$A_DOMAIN" ] && [ "${USE_EXTERNAL_ARGO:-false}" = "true" ]; then
        ARGO_IN=$(printf ',{
          ...
          "transport": { "type": "xhttp", "host": "%s", "method": "stream-up" }
        }' "$PSK" "$A_DOMAIN")
    fi
...
display_links() {
    ...
    [ -n "$RAW_ARGO_DOMAIN" ] && [ "$RAW_ARGO_DOMAIN" != "null" ] && LINK_ARGO="vless://$RAW_PSK@$RAW_ARGO_DOMAIN:443?encryption=none&security=tls&sni=$RAW_ARGO_DOMAIN&type=xhttp&host=$RAW_ARGO_DOMAIN&mode=stream-up&fp=chrome#${hostname_tag}_Argo" && echo -e "\n\033[1;33m[Argo 隧道]\033[0m\n$LINK_ARGO" && FULL_CLIP="${FULL_CLIP:+$FULL_CLIP$'\n'}$LINK_ARGO"
