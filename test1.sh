...
display_links() {
    ...
    _do_probe() {
        ...
        [ -z "$target" ] && return
        netstat -tuln | grep -q "[: ]${RAW_PORT} " && res="\033[1;32m[服务活跃]\033[0m" || res="\033[1;31m[服务离线]\033[0m"
        nc -z -u -w 1 $([ "$mode" = "6" ] && echo "-6") "$target" "$RAW_PORT" >/dev/null 2>&1 && res="${res} \033[1;32m(已放行)\033[0m" || res="${res} \033[1;33m(受阻)\033[0m"
        echo "$res"
    }
