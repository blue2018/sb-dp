...
create_config() {
    ...
    if [ -n "$A_TOKEN" ] && [ -n "$A_DOMAIN" ] && [ "${USE_EXTERNAL_ARGO:-false}" = "true" ]; then
        ARGO_IN=$(printf ',{
          ...
          "transport": { "type": "httpupgrade", "host": "%s", "max_early_data": 2048, "early_data_header_name": "Sec-WebSocket-Protocol" }
        }' "$PSK" "$A_DOMAIN")
    fi
...
display_links() {
	...
	[ -n "$RAW_ARGO_DOMAIN" ] && [ "$RAW_ARGO_DOMAIN" != "null" ] && LINK_ARGO="vless://$RAW_PSK@$RAW_ARGO_DOMAIN:443?encryption=none&security=tls&sni=$RAW_ARGO_DOMAIN&type=httpupgrade&host=$RAW_ARGO_DOMAIN&path=%2F%3Fed%3D2048&eh=Sec-WebSocket-Protocol&fp=chrome#${hostname_tag}_Argo" && echo -e "\n\033[1;33m[Argo 隧道]\033[0m\n$LINK_ARGO" && FULL_CLIP="${FULL_CLIP:+$FULL_CLIP$'\n'}$LINK_ARGO"
