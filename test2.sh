...
install_dependencies() {
    info "正在检查系统类型..."
    local PM=""; 
    # 核心运行依赖：保持你原有的，新增 kmod (BBR必备) 和 resolvconf (WARP/DNS必备)
    local COMMON_DEPS="curl jq openssl ca-certificates iproute2 ethtool iptables bash tzdata tar kmod resolvconf"
    local WARP_DEPS="wireguard-tools"
    local DEPS=""

    if command -v apk >/dev/null 2>&1; then 
        PM="apk"; DEPS="${COMMON_DEPS} ${WARP_DEPS} netcat-openbsd procps"
    elif command -v apt-get >/dev/null 2>&1; then 
        PM="apt"; DEPS="${COMMON_DEPS} ${WARP_DEPS} netcat-openbsd procps util-linux"
    elif command -v yum >/dev/null 2>&1 || command -v dnf >/dev/null 2>&1; then 
        PM="yum"; DEPS="${COMMON_DEPS} ${WARP_DEPS} nc procps-ng util-linux"
    else 
        err "未检测到支持的包管理器 (apk/apt-get/yum)，请手动安装依赖"; exit 1
    fi

    case "${PM}" in
        apk)
            info "检测到 Alpine 系统，正在同步仓库并分批安装依赖..."
            apk update >/dev/null 2>&1
            # 关键：原子化分批安装，解决内存溢出 Killed 问题
            for pkg in ${DEPS}; do
        ...
        apt)
            ...
            apt-get install -y --no-install-recommends ${DEPS} || {
                for pkg in ${DEPS}; do apt-get install -y --no-install-recommends "$pkg" >/dev/null 2>&1 || true; done
            }
            apt-get clean
            ;;
        yum)
            ...
            ${M} install -y ${DEPS} || {
                for pkg in ${DEPS}; do ${M} install -y "$pkg" >/dev/null 2>&1 || true; done
            }
            ${M} clean all
            ;;
    esac

    # 针对你脚本中使用的 stat/pgrep 命令进行高兼容补漏
    for cmd in stat pgrep; do
        if ! command -v "${cmd}" >/dev/null 2>&1; then
            if [ "${PM}" = "apk" ]; then
                [ "$cmd" = "stat" ] && apk add --no-cache coreutils >/dev/null 2>&1 || true
                [ "$cmd" = "pgrep" ] && apk add --no-cache procps >/dev/null 2>&1 || true
            fi
        fi
    done

    [ -d /usr/share/zoneinfo ] || { [ "${PM}" = "apk" ] && apk add --no-cache tzdata >/dev/null 2>&1; }
    update-ca-certificates 2>/dev/null || true
    
    # 你的核心校验逻辑，确保脚本主体不崩溃
    for cmd in jq curl tar bash pgrep; do 
        command -v "${cmd}" >/dev/null 2>&1 || { err "核心依赖 ${cmd} 安装失败，请检查网络"; exit 1; }
    done
    succ "所需依赖已就绪 (含 WARP 预埋组件)"
}
