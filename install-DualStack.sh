...
install_sbox_kernel() {
    ...
    if [ "$is_update" = "true" ]; then
        ...
        info "正在部署内核: ${LATEST_VER} ($SBOX_ARCH)..."
    fi
    ...
    if [ "$is_update" = "true" ]; then succ "内核更新完成！"; fi
    return 0
}
...
read_port() {
    ...
    local port
    while true; do
        read -r -p "$prompt (直接回车随机 $default): " port
        port=$(echo "$port" | xargs echo -n 2>/dev/null || echo "$port")
        port=${port:-$default}
...
show_nodes() {
    ...
    echo -e "SingBox内核: \033[1;33m$SB_VER\033[0m | 内存优化: \033[1;33m$SBOX_OPTIMIZE_LEVEL\033[0m"
    ...
    case "\$opt" in
        1)
            ...
            if [[ -n "\$HAS_HY2" && -n "\$HAS_ARGO" ]]; then
                echo -e "\n\033[1;33m[提示] Hysteria2 和 VLESS+Argo 协议均已安装。\033[0m"
                read -p "按回车继续..."
                continue
            fi
            echo -e "\n--- 可添加协议 ---"
        ...
        6) 
            ...
            if [[ "\$un_confirm" =~ ^[Yy]$ ]]; then
                ...
                pkill cloudflared || true
                rm -rf /etc/sing-box /usr/bin/sing-box /usr/local/bin/sb /usr/local/bin/SB /usr/bin/cloudflared /etc/sysctl.d/99-singbox-*.conf
